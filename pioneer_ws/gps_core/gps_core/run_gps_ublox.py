...
class EnhancedReadGPS(Node):
    ...
    def timer_callback(self):
        ...
        try:
            ...
            if not self.connection_healthy or not self.gps:
            ...
            lat = coords.lat
            lon = coords.lon
            status = coords.fixType
            satellites = coords.numSV
            ...
            if self._validate_gps_data(lat, lon):
                ...
                msg.altitude = float(coords.hMSL) if coords.hMSL else 0.0
        ...
        try:
            self.get_logger().debug('Published GPS message')
            self.get_logger().debug(f'GPS Message: timestamp, {msg.header.stamp}, lat={msg.latitude}, lon={msg.longitude}, status={msg.status.status}, satellites={msg.status.service}, altitude={msg.altitude}')
            self.gps_publisher.publish(msg)
        except Exception as e:
