...
class Controller(Node):
    ...
    def _create_robot_topics(self):
        ...
        self.robot_hardware = []
        for robot_id in self.robot_id_list:
            self.robot_hardware.append(False)
            # Subscriptions
            self.pubsub.create_subscription(
                Pose2D, f'/{robot_id}/pose2D',
                lambda msg, rid=robot_id: self._robot_pose_callback(msg, rid, True), 5
            )
            self.pubsub.create_subscription(
                Pose2D, f'/sim/{robot_id}/pose2D',
                lambda msg, rid=robot_id: self._robot_pose_callback(msg, rid, False), 5
            )
            self.pubsub.create_subscription(
                Bool, f'/{robot_id}/hardware',
                lambda msg, rid=robot_id: self._robot_hardware_callback(msg, rid), 5
            )
    ...
    def _robot_pose_callback(self, msg: Pose2D, robot_id: str, is_actual: bool):
        """Process robot pose updates"""
        if not self.robot_hardware[self.robot_id_list.index(robot_id)] == is_actual:
            return
        try:
            new_pose = [float(msg.x), float(msg.y), float(msg.theta)]
            
            # Get appropriate status dictionary
            status_dict = self.robot_status if is_actual else self.robot_status
    ...
    def _robot_hardware_callback(self, msg: Bool, robot_id: str):
        """Process robot hardware status updates"""
        # Placeholder for hardware status processing
        self.robot_hardware[self.robot_id_list.index(robot_id)] = msg.data
    ...
    def _publish_zero_velocities(self, cluster_robots: List[str]):
        ...
        for robot_id in cluster_robots:
            self.pubsub.publish(f"{'' if self.robot_hardware[self.robot_id_list.index(robot_id)] else '/sim'}/{robot_id}/cmd_vel", zero_vel)
    ...
    def _compute_and_publish_robot_commands(self, cluster_robots: List[str], 
        ...
        for i, robot_id in enumerate(cluster_robots):
            ...
            try:
                topic = f"{'' if self.robot_hardware[self.robot_id_list.index(robot_id)] else '/sim'}/{robot_id}/cmd_vel"
    ...
    def _publish_cluster_info(self, cluster_current: np.ndarray, 
        ...
        self.pubsub.publish(f"/cluster_info", cluster_msg)
