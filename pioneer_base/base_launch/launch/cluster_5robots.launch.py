...
def generate_launch_description():
    """
    Integrated launch file for 5-robot cluster with hardware.
    This combines all necessary components for full cluster operation.

    Components:
    - Cluster base controller (5 robots config)
    ...
    return LaunchDescription([
        # Core controller (5 robots)
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(cluster_base_launch),
            launch_arguments={'robot_count': '5'}.items(),
        ),
        # Visualization (hardware mode)
        IncludeLaunchDescription(
