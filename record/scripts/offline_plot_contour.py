...
class ExtractClusterData:
    ...
    def get_centroid_phase_portrait(
        ...
        x_c = np.mean([self._p1_pose2D_x, self._p2_pose2D_x, self._p3_pose2D_x], axis=0)
        y_c = np.mean([self._p1_pose2D_y, self._p2_pose2D_y, self._p3_pose2D_y], axis=0)
        print(x_c.shape)
        fig = self._get_fig(ind, start, end, x_c, y_c, use_arrows= True, color="green")
    ...
    def plot_contour_map(self, ind=1) -> plt.Figure:
        ...
        # Drop NaN
        _p1_pose2D_x = self._p1_pose2D_x[~np.isnan(self._p1_rssi_data)]
        _p2_pose2D_x = self._p2_pose2D_x[~np.isnan(self._p2_rssi_data)]
        _p3_pose2D_x = self._p3_pose2D_x[~np.isnan(self._p3_rssi_data)]

        _p1_pose2D_y = self._p1_pose2D_y[~np.isnan(self._p1_rssi_data)]
        _p2_pose2D_y = self._p2_pose2D_y[~np.isnan(self._p2_rssi_data)]
        _p3_pose2D_y = self._p3_pose2D_y[~np.isnan(self._p3_rssi_data)]


        _p1_rssi_data = self._p1_rssi_data[~np.isnan(self._p1_rssi_data)]
        _p2_rssi_data = self._p2_rssi_data[~np.isnan(self._p2_rssi_data)]
        _p3_rssi_data = self._p3_rssi_data[~np.isnan(self._p3_rssi_data)]


        z= np.concatenate([contour_data._p1_rssi_data, _p2_rssi_data, _p3_rssi_data])
        x= np.concatenate([contour_data._contour_x, _p2_pose2D_x, _p3_pose2D_x])
        y = np.concatenate([contour_data._contour_y, _p2_pose2D_y, _p3_pose2D_y])
        
        # print(self._p1_rssi_data)
        ...
        fig, ax = plt.subplots(num=ind, subplot_kw={'projection':'3d'})

        # contour_lines = ax.tricontour(triang, z, levels=15, linewidths=0.05, colors='k')
        contour_filled = ax.plot_trisurf(triang, z, cmap='coolwarm')

        # Add conoutr lines
        contour_lines= ax.tricontour(triang, z, levels=10, linewidth=0.1, cmap='coolwarm', colors='k')
